...
async function loadAssets() {
  ...
  loadingText.textContent = 'Fetching sky + rigged character...';
  try {
    ...
    if (assets.hdri) {
      ...
      assets.hdri.dispose?.();
      pmrem.dispose();
      loadingDetail.textContent = 'HDRI + rigged characters loaded.';
    } else {
      loadingDetail.textContent = 'Sky: procedural fallback (HDRI unavailable).';
    }
    if (!assets.soldier) {
      loadingDetail.textContent += ' (rig offline — fallback skeletons)';
    }
    return assets;
  } catch (err) {
...
// Populated after asset load so we can pass the rigged Soldier.glb into
// every Skeleton instance (SkeletonUtils.clone + AnimationMixer per enemy).
const enemies = [];
function spawnEnemies(gltfSource) {
  for (const sp of lvl.spawnPoints) {
    enemies.push(new Skeleton(scene, sp, gltfSource));
  }
}

...
export class Skeleton {
  constructor(scene, spawn, options = {}) {
    ...
    // --- ELITE flag: red, fast, aggressive commanders ---
    this.elite = !!options.elite;
    // Aggression multiplier applied to decision / combat tunables.
    // Normal skeletons use 1.0. Elites use ~5.0 where it makes sense,
    // tempered so they stay fair and readable.
    this.aggression = this.elite ? 5.0 : 1.0;

    this.hp = this.elite ? 160 : 100;
    this.maxHp = this.hp;

    // Weapon state
    this.usePickaxe = false; // if true, melee mode
    this.arrows = this.elite ? 60 : 30;
    this.meleeDamage = this.elite ? 45 : 30;
    this.arrowDamageByZone = this.elite
      ? { head: 45, torso: 30, legs: 15 }
      : { head: 30, torso: 20, legs: 10 };
    ...
    // Elites draw bow / ready axe much faster
    this.aimSpeed = this.elite ? 3.0 : 1.2;
    // First-shot cooldown is short for elites so they open fire fast
    this.cooldownShoot = this.elite ? rand(0, 0.3) : rand(0, 1.2);
    this.meleeCooldown = 0;
    this.meleeActive = false;
    this.meleeSwingT = 0;
    this.meleeSwingDur = this.elite ? 0.35 : 0.55; // faster chops
    ...
    // Elites are a bit larger and noticeably faster
    if (this.elite) this.scale *= 1.08;
    this.group.scale.setScalar(this.scale);
    this.speedMul = (0.95 + Math.random() * 0.15) * (this.elite ? 1.5 : 1.0);
  ...
  _buildBody() {
    ...
    const mat = matBone();
    if (elite) {
      // Blood-red bone tone for elite commanders
      mat.color.setHex(0x7a1818);
      mat.emissive = new THREE.Color(0x2a0606);
      mat.emissiveIntensity = 0.35;
    }
    const dirtyMat = matBone();
    dirtyMat.color.setHex(elite ? 0x4a0e0e : 0x8b7d5d);
    const clothMat = new THREE.MeshStandardMaterial({
      color: elite ? 0x5a0a0a : 0x2b2217,
      roughness: 1.0, metalness: 0,
    });
    const armorMat = new THREE.MeshStandardMaterial({
      color: elite ? 0x2a0808 : 0x3b2e1a,
      roughness: 0.7, metalness: elite ? 0.55 : 0.3,
      emissive: elite ? new THREE.Color(0x300808) : new THREE.Color(0x000000),
      emissiveIntensity: elite ? 0.3 : 0.0,
    });
    ...
    head.add(jaw);
    // Eyes (glowing) — elites have intense red eyes
    const eyeColor = elite ? 0xff3020 : 0xffcc66;
    const eyeLightColor = elite ? 0xff2010 : 0xffaa55;
    const eyeMat = new THREE.MeshBasicMaterial({ color: eyeColor, transparent: true, opacity: 0.95 });
    for (const ex of [-0.06, 0.06]) {
      const eye = new THREE.Mesh(new THREE.SphereGeometry(elite ? 0.028 : 0.02, 6, 6), eyeMat);
      eye.position.set(ex, 0.02, 0.12);
      head.add(eye);
    }
    const eyeLight = new THREE.PointLight(eyeLightColor, elite ? 1.4 : 0.5, elite ? 6 : 3, 2);
  ...
  _buildWeapons() {
    const elite = this.elite;
    // BOW - held in left hand (at end of left forearm). Elites carry a
    // darker reinforced bow with a glowing string so shots read clearly.
    const bowGroup = new THREE.Group();
    const bowMat = new THREE.MeshStandardMaterial({
      color: elite ? 0x1a0606 : 0x3a2616,
      roughness: 0.9,
      emissive: elite ? new THREE.Color(0x200404) : new THREE.Color(0x000000),
      emissiveIntensity: elite ? 0.3 : 0,
    });
    const stringMat = new THREE.MeshStandardMaterial({
      color: elite ? 0xff5030 : 0xd8cfb5,
      emissive: elite ? new THREE.Color(0x803010) : new THREE.Color(0x000000),
      emissiveIntensity: elite ? 0.9 : 0,
      roughness: 0.5,
    });
    ...
    const haftMat = new THREE.MeshStandardMaterial({
      color: elite ? 0x1a0606 : 0x2e1e10,
      roughness: 0.9,
    });
    const headMat = new THREE.MeshStandardMaterial({
      color: elite ? 0x502020 : 0x2b2723,
      roughness: elite ? 0.45 : 0.65,
      metalness: elite ? 0.85 : 0.6,
      emissive: elite ? new THREE.Color(0x300808) : new THREE.Color(0x000000),
      emissiveIntensity: elite ? 0.4 : 0,
    });
  ...
  update(dt, game) {
    ...
    // Perception — elites have sharper eyes and a wider view cone
    ...
    const viewCone = this.elite ? -0.1 : 0.25;                    // elites: ~110 deg half-cone
    const sightBase = this.elite ? 50 : 28;                        // elites see much further
    const sightMaxDist = sightBase + (player.sprinting ? 4 : 0) - (player.crouching ? 6 : 0);
    ...
    // Awareness — elites snap to full combat readiness almost instantly
    if (canSee) {
      this.awareness = Math.min(1, this.awareness + dt * (this.elite ? 10.0 : 2.5));
      this.lastKnownTarget = player.position.clone();
      this.target = player;
    } else {
      // Elites stay angry longer, decay slower
      this.awareness = Math.max(0, this.awareness - dt * (this.elite ? 0.08 : 0.25));
  ...
  _chooseState(dt, game, distToPlayer, canSee) {
    // Low HP => retreat briefly. Elites never retreat — they fight to the end.
    if (!this.elite && this.hp < 25 && this.state !== STATES.RETREAT && Math.random() < 0.02) {
    ...
    // Combat transitions — elites engage at a much lower awareness threshold
    const engageThreshold = this.elite ? 0.15 : 0.5;
    if (canSee && this.awareness > engageThreshold) {
      // Adaptive: if player-prefers-cover (from game feedback), sometimes flank.
      // Elites flank noticeably more often.
      const flankChance = 0.005 + this.preferFlank * 0.01 + (this.elite ? 0.02 : 0);
      if (this.state !== STATES.FLANK && Math.random() < flankChance) {
  ...
  _sCombat(dt, game, distToPlayer, canSee) {
    ...
    // Face the target — elites track much faster
    const dx = player.position.x - this.position.x;
    const dz = player.position.z - this.position.z;
    const desiredYaw = Math.atan2(-dx, -dz);
    this.yaw = this._approachYaw(this.yaw, desiredYaw, dt * (this.elite ? 8 : 4));

    if (this.usePickaxe) {
      // Melee approach — elites charge faster
      if (distToPlayer > 1.8) {
        this.path = [{ x: player.position.x, z: player.position.z }];
        this.pathIdx = 0;
        this._followPath(dt, (this.elite ? 4.0 : 2.8) * this.speedMul);
      } else {
      ...
      // Elites hold a tighter range (press the player).
      const idealDist = (this.elite ? 7 : 10) + this.preferDistance * 3;
      if (distToPlayer < idealDist - 3) {
        ...
        this.pathIdx = 0;
        this._followPath(dt, (this.elite ? 3.0 : 1.8) * this.speedMul);
      } else {
        this.velocity.x *= 0.75; this.velocity.z *= 0.75;
      }

      // Aim + shoot (requires LOS, cooldown).
      // Elites nock faster and fire far more often.
      if (canSee && this.cooldownShoot <= 0 && this.arrows > 0) {
        this.aimT = Math.min(1, this.aimT + dt * this.aimSpeed);
        if (this.aimT >= 1.0) {
          this._shootArrow(game, player);
          this.aimT = 0;
          this.cooldownShoot = this.elite
            ? (0.5 + Math.random() * 0.4)
            : (2.2 + Math.random() * 0.8);
          this.arrows -= 1;
        }
  ...
  _shootArrow(game, player) {
    ...
    // Lead slightly if player moving (predictive). Elites lead better.
    const leadT = this.elite ? 0.45 : 0.25;
    target.addScaledVector(player.velocity, leadT * 0.5);

    const dir = target.clone().sub(from);
    const dist = dir.length();
    dir.normalize();
    // Inaccuracy — elites shoot much straighter
    const baseSpread = this.elite ? 0.015 : 0.04;
    const distSpread = this.elite ? 0.015 : 0.04;
    const spread = baseSpread + (dist / 40) * distSpread;
    dir.x += (Math.random() - 0.5) * spread;
    dir.y += (Math.random() - 0.5) * spread;
    dir.z += (Math.random() - 0.5) * spread;
    dir.normalize();

    const speed = this.elite ? 48 : 34;
    game.spawnArrow(from, dir.clone().multiplyScalar(speed), this);
    game.audio.bow(this.position);

    // Visible "fire" pulse for animation: arms jolt back, string snaps forward,
    // a brief bright flash on the bow string (for elites).
    this._fireRecoilT = 0.28; // seconds
    if (this.elite && this._bowString) {
      this._bowString.scale.set(1.0, 1.0, 4.0); // wide flash
    }
  }
  ...
  _animate(dt) {
    ...
    // Decay fire-recoil pulse (used to show bow-release kick)
    if (this._fireRecoilT === undefined) this._fireRecoilT = 0;
    this._fireRecoilT = Math.max(0, this._fireRecoilT - dt);
    const fireKick = this._fireRecoilT > 0 ? (this._fireRecoilT / 0.28) : 0; // 1->0
    ...
    if (!this.usePickaxe && (this.aimT > 0.05 || fireKick > 0)) {
      // Hold bow forward, but snap the right arm backward briefly on release.
      this._armL.group.rotation.x = -1.4 + fireKick * 0.25;
      this._armL.group.rotation.y = 0.1;
      this._armL.elbow.rotation.x = 0.1;

      // Right arm: drawing position while aiming; on release, snap forward.
      this._armR.group.rotation.x = -1.2 - this.aimT * 0.3 + fireKick * 0.6;
      this._armR.group.rotation.y = -0.2;
      this._armR.elbow.rotation.x = -1.2 - this.aimT * 0.8 + fireKick * 1.2;

      // Torso leans into shot on release
      if (this._pelvis) this._pelvis.rotation.x = -fireKick * 0.12;
      for (let i = 0; i < this._spine.length; i++) {
        this._spine[i].rotation.x = -fireKick * 0.06 * (i + 1);
      }

      // Draw string while aiming; on release, string snaps forward past rest
      const drawX = 0.08 - this.aimT * 0.18 + fireKick * 0.22;
      this._bowString.position.x = drawX;

      // Reset flash scale (set in _shootArrow) gradually
      if (this._bowString.scale.z > 1.05) {
        this._bowString.scale.z = Math.max(1.0, this._bowString.scale.z - dt * 12);
      }
      ...
      // Torso twist into the chop
      if (this._pelvis) this._pelvis.rotation.y = Math.sin(t * Math.PI) * 0.35;
    } else {
      ...
      if (this._pelvis) this._pelvis.rotation.y = 0;
